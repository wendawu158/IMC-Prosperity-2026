...
class Trader:
    ...
    def intarian_pepper_root(self):
        ...
        if self.state.position.get("INTARIAN_PEPPER_ROOT", 0) < 80:
            self.orders["INTARIAN_PEPPER_ROOT"] = \
                [Order("INTARIAN_PEPPER_ROOT",
                       sorted(self.state.order_depths["INTARIAN_PEPPER_ROOT"].sell_orders.keys())[0],
                       80 - self.state.position.get("INTARIAN_PEPPER_ROOT", 0)
                       )]
