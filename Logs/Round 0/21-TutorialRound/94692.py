...
class Trader:
    ...
    def tomato(self):
        ...
        if best_fair_mm_ask < previous_best_stable_midpoint:
            best_fair_mm_ask = sorted(asks_tomatoes.keys())[1] - 1
            if best_fair_mm_ask < previous_best_stable_midpoint:
                best_fair_mm_ask = sorted(asks_tomatoes.keys())[2] - 1

        if best_fair_mm_bid > previous_best_stable_midpoint:
            best_fair_mm_bid = sorted(bids_tomatoes.keys())[-2] + 1
            if best_fair_mm_bid > previous_best_stable_midpoint:
                best_fair_mm_bid = sorted(bids_tomatoes.keys())[-3] + 1
